...
export default class InspectorService {
    ...
    private currentView: InspectorView = InspectorView.CONTENT;
    private currentShape?: AppElement | AppLink;
    private markerButtonGroup?: ui.SelectButtonGroup;
    private shapeButtonGroup?: ui.SelectButtonGroup;

    private inspectorEl: HTMLDivElement;
    private contentButton: HTMLButtonElement;
    private appearanceButton: HTMLButtonElement;
    // Used for aborting color field event listeners
    private fieldController: AbortController;
    // Used for aborting the `InspectorService` event listeners
    private inspectorAbortController?: AbortController;

    constructor({ inspectorEl, contentButton, appearanceButton }: InspectorElements) {
    ...
    create(shape: AppElement | AppLink) {
    ...
    private createContentView(shape: AppElement | AppLink) {
        ...
        if (shape.isElement() && shape.getMarkers && shape.getMarkers().length > 0) {
            ...
            const shapeMarkers: string[] = shape.get('markers');
    ...
    private createAppearanceView(shape: AppElement | AppLink) {
        ...
        ui.Inspector.create(this.inspectorEl, {
            ...
            renderFieldContent: (opt, path, value, inspector) => {
                ...
                if (opt.type !== 'color') return undefined;
                ...
                colorInput.addEventListener('change', (event) => {
                    ...
                    originalValue = (event.target as HTMLInputElement).value;
                    syncedValue = true;
                }, { signal: this.fieldController.signal });
    ...
    private canCurrentShapeAccessContentView() {
