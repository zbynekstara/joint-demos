...
export abstract class Activity extends shapes.bpmn2.Activity implements AppElement {
    ...
    getShapeList(): string[] {
        ...
        return shapes.filter((shape: string) => shape !== this.get('type'));
    }
    ...
    sortMarkers(markers: MarkerNames[]): MarkerNames[] {
        return markers.sort((markerA: MarkerNames, markerB: MarkerNames) => {
            const markersConfig = this.getMarkers();
            const markerAIdx = markersConfig.find((markerConfig: Marker) => markerConfig.name === markerA);
            const markerBIdx = markersConfig.find((markerConfig: Marker) => markerConfig.name === markerB);
