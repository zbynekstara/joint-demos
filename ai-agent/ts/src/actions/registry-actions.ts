...
// Diagram
import { Agent, Action, Trigger } from '../diagram/models';
// Registry
import { createRegistryKey, resolveActionProvider, resolveTriggerProvider } from '../registry';
import { TriggerOption, ActionOption } from '../registry/models';

import type { App } from '../app';
import type { Node } from '../diagram/models';
import type { AgentSkill } from '../diagram/models/Agent';
 ...
 */
export function openProviderRegistryDialog(app: App, node: Node) {

    let groups: ui.Stencil.Options['groups'];
    let cells: { [groupName: string]: dia.Cell[] };
    let title: string;
    ...
    if (node instanceof Trigger) {
        ...
        ({ cells, groups } = createTriggersOptions(app, {
            // Disable the trigger assigned to this trigger node
            disabled: node.isConfigured() ? [node.getTriggerKey() as string] : []
        }));

        updateShape = (option: TriggerOption) => {
            const triggerKey = option.get('triggerKey');

            node.setTriggerKey(triggerKey, { ui: true });
        };
        ...
        ({ cells, groups } = createActionsOptions(app, {
            ...
            disabled: node.isConfigured() ? [node.getActionKey() as string] : []
        }));
        ...
        ({ cells, groups } = createActionsOptions(app, {
            // Disable actions that are already added as skills
            disabled: Object.keys(node.getSkills())
        }));

        updateShape = (option: ActionOption) => {
            const skill: AgentSkill = {
                id: option.get('actionKey'),
                name: option.attr('label/text'),
                icon: option.attr('icon/href')
            };

            node.addSkill(skill, { ui: true });
        };
    } else {
 ...
 */
export function refreshActionNodeProvider(app: App, node: Action) {
    const { config } = app;
 ...
 */
export function refreshTriggerNodeProvider(app: App, node: Trigger) {
    const { config } = app;

    const definition = resolveTriggerProvider(config.triggers, node.getTriggerKey());
    if (definition) {
        const [provider, trigger] = definition;
        node.updateProvider(provider, trigger);
    } else {
        node.unsetProvider();
    }
}
 ...
 */
function createTriggersOptions(app: App, options: { disabled: string[] } = { disabled: [] }) {
    const { config } = app;
    ...
    for (const providerId in registry) {
        ...
        provider.triggers.forEach((trigger) => {
            const triggerKey = createRegistryKey(providerId, trigger.id);
            const triggerOption = new TriggerOption({
                ...
                attrs: {
                    root: {
                        ...
                        style: options.disabled.includes(triggerKey)
 ...
 */
function createActionsOptions(app: App, options: { disabled: string[] } = { disabled: [] }) {
    const { config } = app;
