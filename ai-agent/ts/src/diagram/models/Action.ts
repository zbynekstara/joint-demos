...
import type { ActionData, ActionDefinition, ActionProviderDefinition } from '../../registry/types';
...
interface ActionAttributes extends LabeledNodeAttributes {
    [Attribute.ActionKey]: string | null;
}

export default class Action extends LabeledNode<ActionAttributes> {

    static type = TYPE;

    static growthLimit = 1;

    dataDefinition: ActionData = {};
    ...
    getActionKey(): string | null {
        return this.get(Attribute.ActionKey) || null;
    }

    setActionKey(actionKey: string, options?: dia.Cell.Options) {
    ...
    updateProvider(provider: ActionProviderDefinition, action: ActionDefinition) {
        const { icon } = provider;
