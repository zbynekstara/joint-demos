...
interface Connectable {
    connectionStrategy(end: dia.Link.EndJSON, coords: g.Point): dia.Link.EndJSON;
}
...
export class CompositeCable extends dia.Element {
    ...
    getSourceEnd() {
        const cable = this.getCableLink() as dia.Link;
    ...
    getSourceWires() {
        ...
        return sourceEnd && sourceEnd.getEmbeddedCells() as dia.Link[];
    }

    getTargetWires() {
        const targetEnd = this.getTargetEnd();
        return targetEnd && targetEnd.getEmbeddedCells() as dia.Link[];
    }
    ...
    static create(graph: dia.Graph, colors: string[], x1: number, y1: number, x2: number, y2: number) {
        ...
        colors.forEach((color: string, index: number, { length: wireCount }) => {
        ...
        [cableSource, cableTarget].forEach((end: dia.Element) => {
    ...
    static isCompositeCable(cell: dia.Cell): cell is CompositeCable {
        return cell instanceof CompositeCable;
    }
}

export class ScrewTerminal extends dia.Element implements Connectable {
    ...
    connectionStrategy(end: dia.Link.EndJSON, coords: dia.Point) {
    ...
    static create(graph: dia.Graph, length: number, x: number, y: number) {
    ...
    static isScrewTerminal(cell: dia.Cell): cell is ScrewTerminal {
        return cell instanceof ScrewTerminal;
    }
}

export class Plug extends dia.Element implements Connectable {
    ...
    connectionStrategy(end: dia.Link.EndJSON) {
        end.anchor = { name: 'modelCenter', args: { dx: 5, dy: 20 }};
        end.connectionPoint = { name: 'anchor' };
        return end;
    }

    static create(graph: dia.Graph, x: number, y: number) {
        const plug = new Plug();
    ...
    static isPlug(cell: dia.Cell): cell is Plug {
