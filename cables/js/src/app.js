...
export const init = () => {
    ...
    const paper = new dia.Paper({
        ...
        validateConnection: function (cellViewS, magnetS, cellViewT, magnetT, end, _linkView) {
            ...
            if (cellViewS === cellViewT)
                return false;
            if (end === 'target' && !cellViewT.model.get('connect'))
                return false;
    ...
    paper.on('element:pointerclick', (elementView) => {
        ...
        const element = elementView.model;
        const [root = element] = element.getAncestors();
    ...
    function findConnectedLinks(graph, element, portId) {
        ...
        return graph.getConnectedLinks(element).filter(link => {
            if (link.source().id === id && link.source().port === portId)
                return true;
            if (link.target().id === id && link.target().port === portId)
                return true;
            return false;
        });
    }
