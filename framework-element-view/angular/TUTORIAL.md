...
<a href="https://stackblitz.com/github/clientio/joint-demos/tree/angular/angular-element-view">
  <img
...
Before creating the Element view, we define a custom Element model whose [markup](https://docs.jointjs.com/learn/features/diagram-basics/cells/#markup) includes a `foreignObject` with an HTML `div` container. This container is where Angular will render.
...
- The `attrs.foreignObject` uses [calc expressions](https://docs.jointjs.com/learn/features/customizing-shapes/relative-dimensions/#relative-dimensions) to size the foreignObject to match the element model size `element.size()`.
...
By default, `update()` is only called when so called "presentation attributes" change, such as `attrs` or `markup`. To react to changes in the model's `data` property, we need to add `data` to the [presentation attributes](https://docs.jointjs.com/api/dia/CellView#presentationAttributes).
...
- Register the view in `cellViewNamespace` with the naming convention `{ElementType}View` (For more info, see [docs](https://docs.jointjs.com/learn/features/customizing-shapes/cell-namespaces/#adding-custom-views))
...
- Instead of creating a brand-new element, you can extend an existing JointJS shape, add a `foreignObject` to its markup, and render an Angular component only for a specific part of the element (for example, a custom label or inspector area).
- Angular components that render SVG are also supported. In that case, the root `g` SVG element can be used as the host element for `createComponent()`.
