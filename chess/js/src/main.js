...
const KingBlack = dia.Element.define('chess.KingBlack', {
    size: { width: 42, height: 38 }
}, {
    markup: util.svg/*xml*/`
        <g @selector="scalable">
...
const QueenWhite = dia.Element.define('chess.QueenWhite', {
    size: { width: 42, height: 38 },
    attrs: {
        root: {
            cursor: 'move'
...
const QueenBlack = dia.Element.define('chess.QueenBlack', {
    size: { width: 42, height: 38 }
}, {
    markup: util.svg/*xml*/`
        <g @selector="scalable">
...
const RookWhite = dia.Element.define('chess.RookWhite', {
...
const RookBlack = dia.Element.define('chess.RookBlack', {
    size: { width: 32, height: 34 }
}, {
    markup: util.svg/*xml*/`
        <g @selector="scalable">
...
const BishopWhite = dia.Element.define('chess.BishopWhite', {
...
const BishopBlack = dia.Element.define('chess.BishopBlack', {
    size: { width: 38, height: 38 }
}, {
    markup: util.svg/*xml*/`
        <g @selector="scalable">
...
const KnightWhite = dia.Element.define('chess.KnightWhite', {
...
const KnightBlack = dia.Element.define('chess.KnightBlack', {
    size: { width: 38, height: 37 }
}, {
    markup: util.svg/*xml*/`
        <g @selector="scalable">
...
const PawnWhite = dia.Element.define('chess.PawnWhite', {
...
const PawnBlack = dia.Element.define('chess.PawnBlack', {
    size: { width: 28, height: 33 }
}, {
    markup: util.svg/*xml*/`
        <g @selector="scalable">
...
const Board = dia.Paper.extend({

    options: util.assign(dia.Paper.prototype.options, {

        letters: ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h'],

        cellViewNamespace: cellNamespace,
        namespace: cellNamespace.chess,
    ...
    initialize: function() {
        ...
        this.model = new dia.Graph({}, { cellNamespace: cellNamespace });

        dia.Paper.prototype.initialize.apply(this, arguments);
    ...
    reset: function() {
        ...
        util.forIn(this.options.startup, this.addPiece.bind(this));
    },
    ...
    movePiece: function(from, to, opts) {
        ...
        if (!this.options.animation || opts.animation === false) {
            ...
            util.invoke(pc, 'set', 'position', this._n2p(to));

        } else {

            util.invoke(pc, 'transition', 'position', this._n2p(to), {
    ...
    addPointer: function(from, to) {
        ...
        var pointer = new shapes.standard.Link({
    ...
    addPointers: function(from, toArray) {
        ...
        var groupedMoves = util.groupBy(moves, function(p0) {
            return g.Point(p0).theta(p1);
        });
        util.toArray(groupedMoves).map(function(group) {
            var distance = 0;
            var to = null;
    ...
    removePointers: function() {
        ...
        util.invoke(this.model.getLinks(), 'remove');
    },
...
const Chessboard = Board.extend({
    ...
    playMove: function(transition, mv) {
        ...
        if (mv & window.moveflagPromotion) {
            ...
            util.invoke(this.at(to[0] + from[1]), 'remove');
        }
