...
import type { FishboneData } from './data';

const fontFamily = 'Arial';
const rootStyles = getComputedStyle(document.documentElement);

export function generateFishboneCells(data: FishboneData): Array<dia.Cell> {

    const elements: Array<dia.Element> = [];
    const links: Array<dia.Link> = [];

    function iterate(obj: FishboneData, pathArray: Array<string|number> = [], direction: 'top' | 'bottom' = null): dia.Element {
    ...
    const cells: Array<dia.Cell> = [...elements, ...links];
    return cells;
}

export function createNode(name: string, level: number, direction: 'top' | 'bottom' = 'bottom'): dia.Element {
...
function createHeadNode(name: string): dia.Element {
...
function createTailNode(name: string): dia.Element {
    const textSize = measureTextSize(name, 22, fontFamily);
    const textMargin = 40;
    return new shapes.standard.Path({
        size: {
...
function createCauseNode(name: string): dia.Element {
    const fontSize = 16;
...
function createEffectNode(name: string): dia.Element {
...
function createHorizontalSubCauseNode(name: string, _level: number, _direction: 'top' | 'bottom'): dia.Element {
    const fontSize = 10;
    const textSize = measureTextSize(name, fontSize, fontFamily);
    const textMargin = Number.parseFloat(rootStyles.getPropertyValue('--text-margin'));
...
function createDiagonalSubCauseNode(name: string, _level: number, direction: 'top' | 'bottom'): dia.Element {
