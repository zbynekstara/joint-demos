...
export const init = () => {
    ...
    const runLayout = function() {
        const getRangeValue = (widget: ui.Widget) => {
            return (<ui.widgets.range>widget).getValue();
        };
    ...
    function selectNode(paper: dia.Paper, selection: string) {
    ...
    paper.on('element:pointerclick', (elementView) => {
        ...
        const nameInput = inspector.el.querySelector('.text') as HTMLDivElement | null;
        if (nameInput) {
            nameInput.focus();
