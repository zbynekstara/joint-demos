...
export const init = () => {
    ...
    const paper = new dia.Paper({
        ...
        validateConnection: (cellViewS, _magnetS, cellViewT, _magnetT, _end, _linkView) => {
            ...
            if (shapes.bpmn2.Swimlane.isSwimlane(source) || shapes.bpmn2.Swimlane.isSwimlane(target))
                return false;
        ...
        interactive: (cellView) => {
            ...
            if (shapes.bpmn2.Phase.isPhase(cell))
                return { stopDelegation: false };
            return true;
        },
        validateEmbedding: (childView, parentView) => {
            ...
            if (shapes.bpmn2.CompositePool.isPool(child))
                return false;
            if (shapes.bpmn2.Swimlane.isSwimlane(child))
                return false;
            ...
            if (!shapes.bpmn2.Swimlane.isSwimlane(parent))
                return false;
            return true;
        },
    ...
    paper.on('element:pointerdblclick', (elementView, evt) => {
        ...
        if (shapes.bpmn2.CompositePool.isPool(element) || shapes.bpmn2.Swimlane.isSwimlane(element) || shapes.bpmn2.Phase.isPhase(element)) {
            ...
            const { target } = evt;
            if ((target.tagName !== 'tspan') && (target.getAttribute('joint-selector') !== 'header'))
                return;
        }
