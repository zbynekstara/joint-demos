...
export function editElementLabel(elementView: dia.ElementView) {
    ...
    let setTextarea = (textarea: HTMLTextAreaElement) => {
    ...
    if (!bbox) {
        const node = elementView.findNode(bgSelector) as SVGElement;
    ...
    if ((element instanceof shapes.bpmn2.HeaderedHorizontalPool) || (element instanceof shapes.bpmn2.HeaderedVerticalPool)) {
        ...
        const headerSide = (element as shapes.bpmn2.Swimlane | shapes.bpmn2.Phase).getHeaderSide();
        if (headerSide === 'left' || headerSide === 'right') {
            bbox.rotateAroundCenter(-90);
            rotate = true;
        }
...
function editText(paper: dia.Paper, element: dia.Element, bbox: g.Rect, basePath: string, options: EditTextOptions) {
    const {
        bodySelector = 'body',
        labelSelector = 'label',
        textAttrPath,
