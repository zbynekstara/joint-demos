...
export function getItemIcon(name: string, color: string = null) {
    let icon = icons[name];
    if (!icon) throw new Error(`Icon "${name}" not found`);
    if (color) icon = icon.replace('currentColor', color);
...
export function blendWithWhite(hex: string, opacity: number): string {
    ...
    const blend = (c: number) => Math.round(opacity * c + (1 - opacity) * 255);
