...
export default class PertChart extends mvc.View<undefined, HTMLElement> {

    graph: dia.Graph;
    paper: dia.Paper;
    scroller: ui.PaperScroller;
    toolbar: ui.Toolbar;
    navigator: ui.Navigator | null;
    ...
    addClickEventListener(callback: (id: TaskId | null, evt: dia.Event) => void) {
        this.paper.on('element:pointerclick', (elementView: dia.ElementView, evt) => {
    ...
    protected onCellAdded(cell: dia.Cell) {
        if (cell.isLink()) return;
        const elementView = this.paper.findViewByModel(cell) as dia.ElementView;
    ...
    protected createCells(data: TaskData[] = []) {
