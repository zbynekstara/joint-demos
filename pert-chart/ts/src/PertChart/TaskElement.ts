import {
    TASK_COLORS, TASK_BG_COLOR, TASK_FONT_SIZE, TASK_HEADER_HEIGHT, TASK_PADDING,
    TASK_ROW_MARGIN, TASK_TEXT_COLOR, TASK_TEXT_DISABLED_COLOR,
    TASK_WIDTH, TASK_BORDER_RADIUS,
    PROGRESS_FONT_SIZE, PROGRESS_LABEL_WIDTH, PROGRESS_MARGIN, PROGRESS_ROW_HEIGHT,
    ASSIGNEE_FONT_FAMILY, ASSIGNEE_FONT_SIZE, ASSIGNEE_GAP, ASSIGNEE_ICON_WIDTH,
...
export class TaskElement extends Record {
    ...
    resetLayout(opt: dia.Cell.Options = {}) {
    ...
    runLayout() {
        ...
        const assigneesWidth =
            assigneeLayouts.reduce((acc, assignee) => Math.max(acc, assignee.rightX), 0) - x0;
        const assigneesHeight =
            assigneeLayouts.reduce((acc, assignee) => Math.max(acc, assignee.bottomY), y0) - y0;
    ...
    getAssignees(): TaskResource[] {
        return this.get('assignees') || [];
    }

    addAssignee(assignee: TaskResource) {
        const assignees = this.getAssignees();
    ...
    static getEndDate(startDate: Date, duration: number) {
        const endDate = new Date(startDate);
        endDate.setDate(endDate.getDate() + duration);
        return endDate;
    }

    static fromData(data: TaskData): TaskElement {
        const {
            id,
            name = '',
            assignees,
        ...
        let color: string, secondaryColor: string;
        if (userColor) {
