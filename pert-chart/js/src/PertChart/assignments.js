...
function onDrag(ctx, taskView, evt, dropTarget) {
    ...
    if (targetTask) {
        ...
        evt.data.targetTask = targetTask;
    }
    else {
...
function onDragEnd(ctx, taskView, evt) {
    ...
    const { sourceTask, targetTask, overlay } = evt.data;
...
function loadResources(stencil, resources) {
    ...
    stencil.getGraph().getElements().forEach((el) => {
        StencilHoverHighlighter.add(el.findView(stencilPaper), 'root', 'stencil-highlight', {
