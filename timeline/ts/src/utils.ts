...
function routeOneDirection(tree: layout.TreeLayout, sourceElement: dia.Element, links: dia.Link[], direction: 'T' | 'B') {
    ...
    const gap = Math.min(
        idealGap,
        Math.floor((tree.get('parentGap') - bundleParentGap * 2) / Math.max(leftLinkCount, rightLinkCount)), // vertical check
...
function getTree(graph: dia.Graph, element: dia.Element) {
    const elements = [element];
    graph.search(element, (cell) => {
        if (cell === element) return true;
        if (cell.get('busElement')) return false;
        elements.push(cell);
        return true;
    }, { outbound: true });
    return elements;
...
function getConnectedBusLinks(graph: dia.Graph, element: dia.Element, outbound: boolean) {
    return graph
        .getConnectedLinks(element, { [outbound ? 'outbound' : 'inbound']: true })
        .filter((link) => link.get('busLink'));
}

function addBusElementAfter(graph: dia.Graph, element: dia.Element) {
...
export function removeElement(tree: layout.TreeLayout, element: dia.Element, removeBranch = true) {
...
export function getPath(graph: dia.Graph, element: dia.Element) {
...
export function makeLink(
    parentElementLabel: dia.Cell.ID = util.uniqueId(),
    childElementLabel: dia.Cell.ID = util.uniqueId()
) {
...
function updateBus(graph: dia.Graph) {
    ...
    busElements.forEach((id, index) => {
        ...
        if (!hasLinkToNextBus) {
            const target = graph.getCell(busElements[index + 1]) as dia.Element;
...
export function addBusElementTools(tree: layout.TreeLayout, paperScroller: ui.PaperScroller, selection: ui.Selection) {
...
export function layoutDiagram(tree: layout.TreeLayout, paperScroller: ui.PaperScroller, selection: ui.Selection) {
    const paper = paperScroller.options.paper;
...
export function addBusLinkTools(tree: layout.TreeLayout, paperScroller: ui.PaperScroller, selection: ui.Selection) {
    const paper = paperScroller.options.paper;
    const graph = paper.model;

    graph.getLinks().forEach((link) => {
        if (!link.get('busLink')) return;
...
export function layoutWithParallelRouting(graph: dia.Graph, treeLayout: layout.TreeLayout) {
    treeLayout.layout();
