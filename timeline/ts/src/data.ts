...
const artificialIntelligenceTimelineData: TimelineData = {
    milestones: [
        {
            id: '2015',
            children: [
...
class Timeline {
    ...
    addBusElementAfter(year: dia.Cell.ID) {
    ...
    addChildToPath(path: string[], child: TimelineNode, index: number) {
    ...
    getByPath(path: string[]): TimelineNode {
    ...
    setValueByPath(path: string[], value: Partial<TimelineNode>) {
        const identifier = path.pop();
    ...
    moveChildByPath(oldPath: string[], newPath: string[], index: number) {
    ...
    toGraphShapes(nodesToTraverse: TimelineNode[] = this.milestones, parentId: dia.Cell.ID = null, depth: number = 0): dia.Cell[] {
    ...
    private resolveShapeTypeFromDepth(node: TimelineNode, depth: number) {
