...
export function makeElement(node: Node, parent: Node, index: number) {
    const {
        id,
        label = '',
        type = Shapes.Device,
        ...
        connectionStyle = ConnectionStyle.Solid,
        connections = Connections.Parallel
    } = node;
    let element: dia.Element;
    if (!id) throw new Error('The node must have an ID.');
    switch (type) {
        case Shapes.Router:
...
export function makeRouterElement(id: dia.Cell.ID, label: string) {
...
export function makeDeviceElement(id: dia.Cell.ID, label: string) {
...
export function makeVirtualElement(id: dia.Cell.ID, label: string) {
    return new shapes.standard.Rectangle({
        id,
        size: {
            width: EL_WIDTH,
...
export function makeCloudElement(id: dia.Cell.ID, label: string) {
...
export function makeComputerElement(id: dia.Cell.ID, label: string) {
...
export function makeLineElement(id: dia.Cell.ID, label: string) {
    return new shapes.standard.Rectangle({
        id,
...
export function makeLink(parentElement: dia.Element, childElement: dia.Element, {
    direction = 'none',
    style = 'solid'
}) {
...
export function makeBoundaryElement(id: dia.Cell.ID, label: string) {
    return new shapes.standard.Rectangle({
        id,
        attrs: {
            root: {
...
export function getLabelHeight(label: string) {
    if (!label) return 0;
    return label.split('\n').length * LINE_HEIGHT;
}

export function getElementBBox(element: dia.Element) {
    const bbox = element.getBBox();
    const label = element.attr('label');
    if (!label) return bbox;
