import { dia, shapes, highlighters, mvc, ui, type g } from '@joint/plus';
...
export const init = () => {
    ...
    const paper = new dia.Paper({
        ...
        cellVisibility: (cell) => {
            ...
            if (cell.get('highlighted')) return true;
    ...
    inspector.el.addEventListener('focusin', (evt) => {
        ...
        if (!listItemEl) {
            ...
            const fieldEl = listItemEl.querySelector(':scope > [data-field]') as HTMLElement;
        ...
        if (!element) return;
        unhighlightFields(inspector);
        highlightField(inspector, listItemEl, element as dia.Element, { scroll: false });
    });
    ...
    function unhighlightFields(inspector: ui.Inspector) {
    ...
    function clearData() {
        ...
        const root = graph.getCell('root') as dia.Element;
        highlightField(inspector, null, root, { scroll: false });
    }
};


function zoomToFit(paper: dia.Paper, contentArea: g.Rect) {
