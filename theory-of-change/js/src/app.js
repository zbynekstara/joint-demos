...
export const init = () => {
    ...
    const paper = new dia.Paper({
        ...
        validateConnection: (sourceView, _sourceMagnet, targetView, _targetMagnet) => {
            ...
            if (source === target)
                return false;
            if (source.isEmbeddedIn(target) || target.isEmbeddedIn(source))
                return false;
    ...
    const stackLayoutView = new ui.StackLayoutView({
        ...
        validateMoving: ({ targetStack, sourceStack }) => {
            if (targetStack.index !== sourceStack.index)
                return false;
            return true;
        },
    ...
    function buildElements(items, stackIndex, level1Key, level2Key, level3Key, options = {}) {
        ...
        const { level3AddButton = '', level2AddButton = '', level1AddButton = '', maxLevel = 3 } = options;
        items.forEach((level1item, index) => {
            // level 1
