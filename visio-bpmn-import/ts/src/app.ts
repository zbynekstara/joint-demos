...
export const init = async() => {
    ...
    navigationEl.addEventListener('click', ({ target }) => {
        if (navigationEl === target) return;
        renderPage(Number((<HTMLElement>target).dataset.pageId));
    });

    const [page0] = vsdDocument.getPages();
    renderPage(page0.id);

    async function renderPage(pageId: number) {
        appEl.classList.add('loading');
        Array.from(navigationEl.childNodes).forEach((pageEl: HTMLElement) => {
    ...
    function renderPageContent(pageContent: VisioPageContent) {
        ...
        const cells = pageContent.toGraphCells({
            ...
            importShape: (vsdShape) => {
                ...
                switch (shapeName) {
                    case 'Event': {
                        ...
                        const icon: string = (data.getProperty('BpmnTriggerOrResult') || 'none').toLowerCase();
                        const iconType = {
                            'none': 'none',
                            'message': 'message1',
                    ...
                    case 'Task': {
                        ...
                        if (!isCollapsed && activityType) {
                            ...
                            const icon: string = activityType.toLowerCase();
                            const iconType = {
                                'receive': 'receive',
                                'service': 'service',
                                'send': 'send',
                    ...
                    case 'Gateway': {
                        ...
                        const gatewayType: string = data.getProperty('BpmnGatewayType');
            ...
            importConnect: (vsdConnect, sourceElement, targetElement) => {
                ...
                switch (connectionObjectType) {
                    case 'sequence flow': {
                        link = new bpmn2.Flow();
                        const conditionType: string = data.getProperty('BpmnConditionType');
                        if (conditionType) {
