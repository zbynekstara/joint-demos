import { dia, layout, shapes, ui, setTheme } from '@joint/plus';
import './styles.css';

import handleImage from '../images/handle.png';
...
const elementTemplate = new shapes.standard.Rectangle({
...
const linkTemplate = new shapes.standard.Link({
    attrs: {
        line: {
            stroke: '#6a6c8a',
            strokeWidth: 2,
...
function showHalo(view, opt) {
    ...
    const halo = new ui.Halo({
        ...
        handles: DIRECTIONS.map((direction, i) => ({
            ...
            attrs: {
                '.handle': {
                    style: `transform: rotate(' + (i * 45) + 'deg); background-image: url("${ handleImage }")`
                }
            }
        })),
    });
