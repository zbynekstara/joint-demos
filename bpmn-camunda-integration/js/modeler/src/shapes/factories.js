...
import { Task, Send, Receive, Service, User, Script, Manual, BusinessRule, CallActivity, SubProcess, EventSubProcess, HttpConnector } from './activity/activity-shapes';
import { Start, MessageStart, TimerStart, ConditionalStart, SignalStart, IntermediateThrowing, MessageIntermediateThrowing, LinkIntermediateThrowing, SignalIntermediateThrowing, EscalationIntermediateThrowing, CompensationIntermediateThrowing, MessageIntermediateCatching, TimerIntermediateCatching, ConditionalIntermediateCatching, LinkIntermediateCatching, SignalIntermediateCatching, IntermediateBoundary, MessageIntermediateBoundary, TimerIntermediateBoundary, ConditionalIntermediateBoundary, SignalIntermediateBoundary, ErrorIntermediateBoundary, EscalationIntermediateBoundary, CompensationIntermediateBoundary, CancelIntermediateBoundary, MessageIntermediateBoundaryNonInterrupting, TimerIntermediateBoundaryNonInterrupting, ConditionalIntermediateBoundaryNonInterrupting, SignalIntermediateBoundaryNonInterrupting, EscalationIntermediateBoundaryNonInterrupting, End, MessageEnd, SignalEnd, ErrorEnd, EscalationEnd, TerminationEnd, CompensationEnd } from './event/event-shapes';
...
class ExportableFlow extends exportableObjects.Flow {
    ...
    constructor(cellView, label, type, conditionExpression) {
    ...
    toFlowXMLElement() {
        ...
        // Set the condition expression text content if provided
        // Camunda 8 Zeebe requires FEEL expressions to start with '='
        if (this.conditionExpression) {
            let expression = this.conditionExpression.trim();
            if (!expression.startsWith('=')) {
                expression = '=' + expression;
            }
            conditionalElement.textContent = expression;
        }
...
export const bpmnExportOptions = {
    exportableObjectFactories: {
        ...
        [ActivityShapeTypes.HTTP_CONNECTOR]: (cellView) => {
            return new ExportableActivity(cellView);
        },
        // Annotation
        ...
        [AnnotationShapeTypes.LINK]: (cellView) => {
            return new exportableObjects.AnnotationLink(cellView);
        },
        // Data
        ...
        [DataShapeTypes.DATA_ASSOCIATION]: (cellView) => {
            return new exportableObjects.DataAssociation(cellView);
        },
        // Events - start
        ...
        [FlowShapeTypes.SEQUENCE]: (cellView) => {
            const conditionExpression = cellView.model.get('conditionExpression');
            // If sequence flow has a condition expression, export as conditional flow
            if (conditionExpression) {
                return new ExportableFlow(cellView, cellView.model.prop('labels/0/attrs/label/text'), 'conditional', conditionExpression);
            }
            return new exportableObjects.Flow(cellView, cellView.model.prop('labels/0/attrs/label/text'));
        },
        ...
        [FlowShapeTypes.MESSAGE]: (cellView) => {
            return new exportableObjects.Flow(cellView, cellView.model.prop('labels/0/attrs/label/text'), 'message');
        },
        // Gateway
...
function getEventType(xmlNode) {
    ...
    return eventDefinition === null || eventDefinition === void 0 ? void 0 : eventDefinition.nodeName.replace('EventDefinition', '').split(':').pop();
}
...
export const bpmnImportOptions = {
    bpmn2Shapes: Object.assign(Object.assign({}, shapes.bpmn2), { HeaderedHorizontalPool: HorizontalPool, HeaderedVerticalPool: VerticalPool, HorizontalSwimlane: HorizontalSwimlane, VerticalSwimlane: VerticalSwimlane, DataAssociation: DataAssociation, Activity: Task, Event: IntermediateThrowing, Gateway: Exclusive, Flow: Sequence, DataObject: DataObject, DataStore: DataStore, Annotation: Annotation, AnnotationLink: AnnotationLink }),
    useLegacyPool: false,
    cellFactories: {
        ...
        serviceTask: (xmlNode, _xmlDoc, _shapeClass, defaultFactory) => {
            ...
            let appElement;
            switch (jointType) {
                case 'activity.HttpConnector': {
                    appElement = new HttpConnector({ id: defaultElement.id });
                    break;
                }
                default: {
                    appElement = new Service({ id: defaultElement.id });
                }
            }
            appElement.copyFrom(defaultElement);
            return appElement;
        ...
        sendTask: (xmlNode, _xmlDoc, _shapeClass, defaultFactory) => {
            ...
            const jointType = xmlNode.getAttribute('joint:type') || '';
            let appElement;
            if (jointType === 'activity.HttpConnector') {
                appElement = new HttpConnector({ id: defaultElement.id });
            } else {
                appElement = new Send({ id: defaultElement.id });
                appElement.setMarkers(getActivityMarkers(xmlNode));
            }
            appElement.copyFrom(defaultElement);
            return appElement;
        ...
        sequenceFlow: (xmlNode, xmlDoc, _shapeClass, defaultFactory) => {
            ...
            const conditionExpressionElement = Array.from(xmlNode.childNodes).find(
                (node) => node.nodeName && node.nodeName.includes('conditionExpression')
            );
            const isConditional = !!conditionExpressionElement;

            const appLink = isConditional ? new Conditional({ id: defaultLink.id }) : new Sequence({ id: defaultLink.id });

            appLink.copyFrom(defaultLink);

            // Extract and set the condition expression text content
            // Strip leading '=' from FEEL expressions for cleaner UI display
            if (conditionExpressionElement && conditionExpressionElement.textContent) {
                let expression = conditionExpressionElement.textContent.trim();
                if (expression.startsWith('=')) {
                    expression = expression.substring(1);
                }
                appLink.set('conditionExpression', expression);
            }
        ...
        dataObject: (xmlNode, _xmlDoc, _shapeClass, defaultFactory) => {
            const appElement = defaultFactory();
