...
export function onPoolDragStart(paper, poolView, evt, _x, _y) {
    var _a, _b, _c;
    ...
    const graphBBox = (_a = graph.getCellsBBox(poolBoundaryElements)) === null || _a === void 0 ? void 0 : _a.inflate(contentMargin).moveAndExpand(moveAndExpandArgs);
    const poolDimensions = new g.Rect(0, 0, Math.max((_b = graphBBox === null || graphBBox === void 0 ? void 0 : graphBBox.width) !== null && _b !== void 0 ? _b : 0, dimensions.width), Math.max((_c = graphBBox === null || graphBBox === void 0 ? void 0 : graphBBox.height) !== null && _c !== void 0 ? _c : 0, dimensions.height));
...
export function onPoolDrag(paper, _poolView, evt, _x, _y) {
...
export function onPoolDragEnd(_paper, _poolView, evt, _x, _y) {
    
    if (!evt.data.poolPreview)
        return;
...
export function onPoolDrop(paper, poolView, evt, _x, _y) {
    ...
    if (!evt.data.poolDropCoordinates)
        return;
...
function calculatePoolDimensions(pool) {
...
function constructPoolPreview(pool, poolDimensions) {
    const poolHeaderSize = pool.getHeaderSize();
...
function isPoolBoundaryRequired(element) {
...
function ensurePoolDragBoundary(encapsulatedBoundary, poolDimensions) {
