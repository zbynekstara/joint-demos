...
import type { TriggerView } from '../diagram/views';
...
function onTriggerAddCriteriaPointerClick(app: App, triggerView: TriggerView) {
    const trigger = triggerView.model;
    openProviderRegistryDialog(app, trigger);
}

function onTriggerRemoveCriteriaPointerClick(app: App, triggerView: TriggerView, evt: dia.Event, criteriaId: string) {
    const trigger = triggerView.model;
