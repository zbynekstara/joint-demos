 ...
 */
export function appendNodeToButtonParent(diagram: Diagram, data: NodeData, button: dia.Element): Node {
 ...
 */
export function replacePlaceholderWithNode(diagram: Diagram, placeholderId: dia.Cell.ID, data: NodeData): Node {
    const { diagramData, graph } = diagram;

    diagramData.createNode(data, placeholderId);
    return graph.getCell(placeholderId) as Node;
}
