...
export default class TriggerView extends AnimatedElementView<Trigger> {

    criteriaSVGElements: Record<string, SVGElement> = {};
    ...
    protected renderCriteria(triggerCriteria: TriggerCriteria): SVGElement {
        const { fontSize, fontFamily } = CRITERIA_TYPOGRAPHY;

        const criteriaName = util.breakText(triggerCriteria.name,
            { width: CRITERIA_WIDTH - Theme.TriggerCriteriaIconSize - Theme.TriggerCriteriaIconMargin * 2 - Theme.TriggerCriteriaDeleteButtonSize },
            { fontSize, fontFamily },
            { maxLineCount: 1, ellipsis: true, svgDocument: this.paper!.svg }
        );
    ...
    events(): mvc.EventsHash {
        return {
            'click .add-criteria-button-container': (evt: dia.Event) => this.onAddCriteriaButtonClick(evt),
            'click .trigger-criteria-remove': (evt: dia.Event) => this.onCriteriaRemoveClick(evt),
        };
    }

    onAddCriteriaButtonClick(evt: dia.Event): void {
        evt.stopPropagation();
        this.paper!.trigger('trigger:add-criteria:pointerclick', this, evt);
    }

    onCriteriaRemoveClick(evt: dia.Event): void {
        evt.stopPropagation();
