import { g, connectors } from '@joint/plus';
 ...
 */
export default function cubicConnector(sourcePoint, targetPoint, routePoints, _, linkView) {
    const opt = {
        cornerType: 'cubic',
        cornerRadius: 12,
    ...
    const path = connectors.straight(sourcePoint, targetPoint, routePoints, opt, linkView);
    
    const segmentsCount = path.segments.length;
    
    for (let i = 0; i < segmentsCount; i++) {
        const segment = path.segments[i];
        if (segment.type === 'M' || segment.type === 'C')
            continue;
...
function lineToCubic(from, to) {
    ...
    return g.Path.createSegment('C', cp1, cp2, to.clone());
}
 ...
 */
function ensureCubicCount(path, targetCurves) {
    const result = path.clone();
    const segments = result.segments;
    
    if (!segments.length || segments[0].type !== 'M')
        return result;
    ...
    if (cubicIndices.length >= targetCurves)
        return result;
