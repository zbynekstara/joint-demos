...
// Text Editing
export function startEditingIdeaLabel(app: App, idea: Idea, event: dia.Event | Event) {
    const { paper, graph } = app;
    const ideaView = idea.findView(paper) as IdeaView;
...
export function openURL(app: App, url: string) {
    if (!url) return;
